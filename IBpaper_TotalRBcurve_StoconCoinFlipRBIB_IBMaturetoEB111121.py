...
def init(cell):
    ...
    cell.geneamt = [0.0, 0.0, 0.0, 0.0]   #hctA-GFP, [1]=Euo, [2]=HctA, [3]=HctB
...
def update(cells):
    ...
    # Celltypes: 0=germ_EB, 1=RBr, 2=RBe, 3=IB, 4=pre_EB, 5=EB, 6=non-dividing RB
    ...
    for (id, cell) in cells.items():
        ...
        #if cell.cellType == 1 and time2 >= 30: #non-dividing RB 
        #    cell.cellType = 6
        #    cell.color = [[1/cell.geneamt[1], 1, 1/cell.geneamt[1]]]
        #    cell.growthRate = 0    
        ...
        if cell.cellType == 2: #RBe
            ...
            if cell.divideFlag == True and random.randint(0, 1) == 0:
                cell.cellType = 3
            #if time2 >= 30: #non-dividing RBe keep for CDI model
            #    cell.cellType = 6
            #    cell.color = [[1/cell.geneamt[1], 1, 1/cell.geneamt[1]]]
            #    cell.growthRate = 0
            #    cell.geneamt[1] = cell.geneamt[1] - (n1 * cell.geneamt[1]) #Euo
        ...
        if cell.cellType == 6: #non-dividing RB
            cell.color = [[1/cell.geneamt[1], 1, 1/cell.geneamt[1]]] 
            cell.growthRate = 0
            cell.geneamt[1] = cell.geneamt[1] - (n1 * cell.geneamt[1]) #Euo
...
# Rbe stochastically decides to convert to IB
